...
BETA_1 = 0.9
BETA_2 = 0.999
CNN_TYPE = "densenet_161" # either "resnet_18", "inception_v3" or "densenet_161"
...
if(__name__ == "__main__"):
    ...
    SAVE_DIR = "../../trained_models/" + CNN_TYPE
    ...
    for data_batch in test_loader: # go through the previously selected test samples
        ...
        if(PERFORMING_BOOTSTRAP):
            ground_truth_all = np.squeeze(ground_truth) if(ground_truth_all is None) else np.concatenate((ground_truth_all, np.squeeze(ground_truth)))
            predicted_all = np.squeeze(predicted_labels) if(predicted_all is None) else np.concatenate((predicted_all, np.squeeze(predicted_labels)))
