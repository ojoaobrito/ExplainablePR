...
# import dataset
from celeba import *
from ubipr_identification import *
from ubipr_verification import *
...
parser.add_argument('--config', default='../../celeba_res101.json', type=str, help='path for the training config file')
...
def main():
    ...
    # wrap to dataset
    '''if args['split'] == 'accuracy':
        train_data = UBIPr_Identification(data_dir, split='train', transform=train_transforms, num_classes=args["num_classes"])
        val_data = UBIPr_Identification(data_dir, split='validation', transform=val_transforms, num_classes=args["num_classes"])
    ...
    # wrap to dataset
    if args['split'] == 'accuracy':
        train_data = UBIPr_Verification(data_dir, split='train', transform=train_transforms)
        val_data = UBIPr_Verification(data_dir, split='validation', transform=val_transforms)
