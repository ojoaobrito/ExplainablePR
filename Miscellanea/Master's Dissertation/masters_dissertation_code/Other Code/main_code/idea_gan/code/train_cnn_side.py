...
if(__name__ == "__main__"):
    ...
    dir_name = "outputs/" + timestamp + "_" + CNN_SIDE_TYPE + "_side"
    os.makedirs(dir_name)
    os.makedirs(dir_name + "/models")
    ...
    elif(CNN_SIDE_TYPE == "inceptionv3"):
    ...
    for i in range(NUM_EPOCHS):
        ...
        if((((i + 1) % MODEL_SAVE_FREQ) == 0) or ((i + 1) == NUM_EPOCHS)):
            ...
            if(OVERWRITE_SAVED_MODELS): # if there is already a saved model, let's remove it
                remove_old_model(dir_name + "/models")

            # save the new model
            torch.save(model.state_dict(), dir_name + "/models/epoch" + str(i + 1) + ".pt")
    ...
    else: plot.save_plot(dir_name + "/training.png")
