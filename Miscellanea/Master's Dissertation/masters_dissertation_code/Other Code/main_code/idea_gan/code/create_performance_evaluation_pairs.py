    ...
    for i in impostor_ID_combinations:
        for j in range(0, len(i), 2):
            ...
            while(True):
                ...
                ID_1 = image_ID_1.split("/")[-2]
                ID_2 = image_ID_2.split("/")[-2]

                # put the chosen images in an appropriate folder
                dir_name = "performance_evaluation_pairs/" + str(dir_count) + "_I" + side[:-1] + "_" + ID_1 + "_" + ID_2
                os.makedirs(dir_name)
                ...
                # ---------------------------------------------------------------------------------------------------------------
                try:
                    base_mask = Image.open("ubipr_masks/" + image_ID_1.split("/")[-1].replace(".jpg", ".png"))
                except: 
                    rmtree(dir_name)
                    continue
                ...
                # copy the masks
                iris_mask.resize((256, 256), Image.LANCZOS).save(dir_name + "/iris_A.png")
                eyebrow_mask.resize((256, 256), Image.LANCZOS).save(dir_name + "/eyebrow_A.png")

                # ---------------------------------------------------------------------------------------------------------------
                # move the masks for the second image
                # ---------------------------------------------------------------------------------------------------------------
                try:
                    base_mask = Image.open("ubipr_masks/" + image_ID_2.split("/")[-1].replace(".jpg", ".png"))
                except: 
                    if(os.path.exists(dir_name)): rmtree(dir_name)
                    continue
                ...
                # copy the masks
                iris_mask.resize((256, 256), Image.LANCZOS).save(dir_name + "/iris_B.png")
                eyebrow_mask.resize((256, 256), Image.LANCZOS).save(dir_name + "/eyebrow_B.png")
                break
...
if(__name__ == "__main__"):
    ...
    for i in directory_10:
        ...
        images_per_ID_10[i] = sample(images_aux_L, 2) + sample(images_aux_R, 2)
    ...
    for i in directory_5:
        ...
        images_per_ID_5[i] = sample(images_aux_L, 2) + sample(images_aux_R, 2)
