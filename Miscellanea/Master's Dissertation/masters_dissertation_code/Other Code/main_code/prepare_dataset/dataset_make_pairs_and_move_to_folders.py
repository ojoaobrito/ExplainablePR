        ...
        for idx, i in enumerate(ids[subset]): # for every ID in that subset
            ...
            if(GENUINE_OR_IMPOSTOR != "I"):
                ...
                for j in possible_good_pairs:
                    ...
                    if((("_L_" in j[0]) and ("_R_" in j[1])) or (("_R_" in j[0]) and ("_L_" in j[1]))): # this pair has images from different sides

                        index_image_keep_side = 0 if((side_that_flips == "_L_" and ("_R_" in j[0])) or (side_that_flips == "_R_" and ("_L_" in j[0]))) else 1
                        ...
                        current_flip = "_R_" if(side_that_flips == "_L_") else "_L_"
                        ...
                        if(("_L_" in j[0]) and ("_L_" in j[1])): ssim_scores.update({"left_left": ssim_scores["left_left"] + [(j[0], j[1], score)]})
                        elif(("_R_" in j[0]) and ("_R_" in j[1])): ssim_scores.update({"right_right": ssim_scores["right_right"] + [(j[0], j[1], score)]})
                ...
                ssim_scores["left_left"] = sample(sorted(ssim_scores["left_left"], key = lambda x : x[2]), len(ssim_scores["left_left"]))
                ssim_scores["right_right"] = sample(sorted(ssim_scores["right_right"], key = lambda x : x[2]), len(ssim_scores["right_right"]))
            ...
            if(GENUINE_OR_IMPOSTOR != "G"):
                ...
                while(count < NUM_GOOD_PAIRS):
                    ...
                    try:
                        base_id_image_side = "_L_" if("_L_" in chosen_base_id_image) else "_R_"
                        other_id_image = list(filter(lambda x : (base_id_image_side) in x, (master_ids_dict[chosen_id[0]][chosen_id[1]]))).pop()
