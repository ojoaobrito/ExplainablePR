...
# the values are coming from the "explain_pair.py" master script
    ...
    TRANSPARENT_BACKGROUND = True if(sys.argv[2] == "True") else False

def assemble_explanation(image_A, image_B, difference_mask_1, difference_mask_2, score, genuine_or_impostor): # auxiliary function, creates the final explanation and saves it
    ...
    # -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
        ...
        final_img[38:37 + IMAGE_SIZE, 284:(284 + (IMAGE_SIZE - 1)), :] = np.asarray(Image.fromarray(difference_mask_1.astype(np.uint8)).resize((IMAGE_SIZE - 1, IMAGE_SIZE - 1)).convert("RGBA"))
        ...
        final_img[38:37 + IMAGE_SIZE, 422:(422 + (IMAGE_SIZE - 1)), :] = np.asarray(Image.fromarray(difference_mask_2.astype(np.uint8)).resize((IMAGE_SIZE - 1, IMAGE_SIZE - 1)).convert("RGBA"))
    ...
    final_img.save("../explanation.png")
...
if(__name__ == "__main__"):
    ...
    assemble_explanation(image_A, image_B, difference_mask_1, difference_mask_2, score, genuine_or_impostor)